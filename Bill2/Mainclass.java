...
public class Mainclass {
	...
	static ArrayList<Bill_pay> paylist= new ArrayList<Bill_pay>();
	static int i=0,j=0,ci=0,ID = 1001,bill_id = 100,bi=0,p=0;
	static int op;
	...
	private static void iniz()
	{
		...
		Inhouse inhouse = new Inhouse("tumbersilver1",15,70);
		list.add(i,inhouse); 
		i++;
		
		inhouse = new Inhouse("tumbersilver2",15,85);
		list.add(i,inhouse); 
		i++;
	...
	private static void newbill()
	{
		...
		while(k1)
		{
			...
			list.get(u).setCount(list.get(u).getCount()-pc);
	...
	public static void printtr()
	{
        ...
        System.out.println("Bill ID|Customer ID|Total|Blance|" );
		for(j=0;j<p;j++)
		{
			System.out.println(paylist.get(j).getBill_id()+"|"+paylist.get(j).getCust_id()+"|"+paylist.get(j).getTotal()+"|"+paylist.get(j).getBlance());
		}
	}
	public static void billprint(int C_id,int bid, String name, String pho)
	{
		double total=0,blance = 0;
		Scanner sc = new Scanner(System.in);
		System.out.println();
        System.out.println();
        System.out.println("-----------------------------------------");
		...
		System.out.println("resvied Amount");
		blance = sc.nextDouble();
		blance = total - blance;
		Bill_pay pay = new Bill_pay(bid,C_id,total,blance);
		paylist.add(p,pay);
		p++;
	}
